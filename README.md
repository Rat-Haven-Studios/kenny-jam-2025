## [Solar Charged](https://hutnerr.itch.io/solarcharged)
Solar Charged was built with Godot for the
[Kenny Game Jam 2025](https://itch.io/jam/kenney-jam-2025) by [Rat Haven Studios](https://github.com/Rat-Haven-Studios).
The theme for the jam was **"Power"**.

It is a resource manager tower defense game, where your towers don't cost money, but require power to keep shooting. 

<img src="https://img.itch.zone/aW1hZ2UvMzczNDQ2OC8yMjMxODcwNC5naWY=/original/PtnoTU.gif" width="600">

## Links
- [Play the game on itch.io](https://hutnerr.itch.io/solarcharged)
- [Jam page](https://itch.io/jam/kenney-jam-2025/rate/3734468)
- hutner's [Post-jam reflection](https://www.hunter-baker.com/pages/games/solar-charged.html) and [Blog Post](https://www.hunter-baker.com/pages/blog/blog-07-24-2025.html)

## How to Play
The goal is to manage your power effectively while defending against waves of enemies. You must strategically place your towers and upgrade them to survive. However, the more you upgrade them, and the more you have, the more power they will use. If you run out of power, you lose.

The game has a weather cycle that must be taken into account as well. Your power is recharged through solar energy, which is affected by the weather. Make sure to keep an eye on the forecast!

To bring up the upgrade menu, simply select a tower that has been placed. You can also delete towers this way.

Upkeep is the cost of having the tower on the field. This cost switches to active when the tower starts shooting.

**Tower Types**
- **Orange Tower**: Simple shooter
- **Red Tower**: Piercing lazer
- **Green Tower**: Increases power of towers in radius
- **Blue Tower**: Slows down enemies by at percent
- **Grey Tower**: Deals constant damage in it's radius

## License
Code is licensed under the MIT License.<br/>
Art, audio, and third-party assets may be subject to different licenses.

## Additional Screenshots
![](https://img.itch.zone/aW1hZ2UvMzczNDQ2OC8yMjIzNDI0MS5wbmc=/original/oHeo15.png)
![](https://img.itch.zone/aW1hZ2UvMzczNDQ2OC8yMjIzNDI0Mi5wbmc=/original/bvbIeK.png)
![](https://img.itch.zone/aW1hZ2UvMzczNDQ2OC8yMjIzNDI0My5wbmc=/original/KgPZpV.png)
![](https://img.itch.zone/aW1hZ2UvMzczNDQ2OC8yMjIzNDY4NC5wbmc=/original/%2F9%2FteC.png)


